...
void *bsp_pm_dump_get(u32 magic, u32 len)
{
	...
	for (i = 0; i < reg_cnt; i++)
	{
		if (g_pm_om_magic_tbl[magic] == entry[i].magic)
		{
	...
	entry[reg_cnt].magic  = g_pm_om_magic_tbl[magic]; /*lint !e661 !e662 */
...
int bsp_pm_om_log_init(void)
{
	...
	{
		g_pmom_ctrl.log.smem = (struct pm_om_smem_cfg *)SHM_MEM_PM_OM_ADDR;
	}
	else
	{
		g_pmom_ctrl.log.smem = (struct pm_om_smem_cfg *)(DDR_SOCP_MEM_VIRT_ADDR + DDR_SOCP_SIZE - PM_OM_SOCP_MEM_SIZE);
	}
...
int bsp_pm_om_dump_init(void)
{
	...
	memset_s((void *)&g_pmom_ctrl.dump, sizeof(g_pmom_ctrl.dump), 0, sizeof(g_pmom_ctrl.dump));

	spin_lock_init(&g_pmom_ctrl.dump.lock);

	g_pmom_ctrl.dump.base = (void *)bsp_dump_register_field(PM_OM_DUMP_ID, PM_OM_DUMP_SIZE);
	...
	memset_s((void *)g_pmom_ctrl.dump.cfg->entry_tbl, sizeof(g_pmom_ctrl.dump.cfg->entry_tbl),
