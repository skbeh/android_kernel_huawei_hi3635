...
{
    return ncm_vendor_open(enDevType, pu32NcmDevId);
}
...
{
    return (BSP_U32)ncm_vendor_write(handle, pPktEncap, net_priv);
}
...
{
    return ncm_vendor_ioctl(handle, enNcmCmd, param);/* [false alarm]:fortify disable */
}
...
{
    return ncm_vendor_close(enDevType, handle);/* [false alarm]:fortify disable */
}
