...
#define UDI_IS_INVALID_TABLE(pstDrvTable) \
    ((BSP_PVOID)(0) == (BSP_PVOID)(pstDrvTable) || (BSP_PVOID)(-1) == (BSP_PVOID)(pstDrvTable))
...
BSP_PVOID BSP_UDI_GetPrivate(UDI_HANDLE handle)
{
	...
	if (UDI_INVALID_HANDLE == handle)
	{
		...
		return (BSP_PVOID)NULL;
	}

	u32Idx = UDI_HANDLE_TO_IDX(handle);

	return g_openNodeTable[u32Idx].pPrivate;
}
