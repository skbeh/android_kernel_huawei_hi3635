...
static s32 icc_send_test_001_cb0(u32 channel_id , u32 len, void* context)
{
	...
	for(i = 0; i < len; i++)
	{
		if((i + start) != (u64)g_icc_test.channels[channel_idx].sub_channels[sub_chn_idx].rd_buf[i])
		{
			confirm = ICC_RECV_ERR;
			goto out;
		}
...
s32 icc_send_test_cb1_new(u32 channel_id , u32 len, void* context)
{
	...
	{
		icc_print_error("icc read error %d\n", confirm);
	}
...
s32 icc_read_task_entry(void *obj)
{
	...
	u32 channel_idx = GET_CHN_ID((u32)(u64)channel_id);
	u32 sub_chn_idx = GET_FUNC_ID((u32)(u64)channel_id);
	s32 read_len    = 0;
	u32 start       = 0;
	u8  confirm     = (u8)ICC_RECV_OK;
	s32 i           = 0;
	...
	{
		channel_id  = obj;
		channel_idx = GET_CHN_ID((u32)(u64)channel_id);
		sub_chn_idx = GET_FUNC_ID((u32)(u64)channel_id);
	}
	...
	for(;g_icc_test.recv_task_flag;)
	{
		...
		read_len = bsp_icc_read((u32)(u64)channel_id, (u8*)buf, ICC_NEW_DATA_LEN);
		...
		icc_print_debug("channel_id=0x%p, confirm=0x%x\n", channel_id, confirm);
...
static s32 icc_test_sub_chn_init(u32 channel_idx, u32 test_sub_chn_size, u32 start)
{
	...
	for(i = 0; i < test_sub_chn_size; i++)
	{
		...
		icc_print_debug("i:0x%x, read_cb: 0x%p", i, read_cb);
...
static s32 icc_test_multi_send_func(void *obj)
{
	u32* channel_id = (u32*)obj;
	u32 i = 0;
	u8 *buf = g_icc_test.channels[GET_CHN_ID((u32)(unsigned long)channel_id)].sub_channels[GET_FUNC_ID((u32)(unsigned long)channel_id)].wr_buf;

	for(i = 0; i < ICC_TEST_SEND_CNT; i++)
	{
		icc_task_delay(200);
		icc_print_debug("try to send channel%d's data, buf: 0x%p", (u32)(unsigned long)channel_id, buf);
		if(ICC_OK != icc_send_test(ICC_SEND_CPU, (u32)(unsigned long)channel_id, buf, 30, GET_FUNC_ID((u32)(unsigned long)channel_id), ICC_TAKE_SEM_TIMEOUT_JIFFIES))
		{
...
{
	u32 channel_id = (u32)((u64)obj & 0xffffffff);
	u32 channel_idx = GET_CHN_ID(channel_id);
	u32 sub_chn_idx = GET_FUNC_ID(channel_id);
	u32 i = 0;
	...
	for(i = 0; i < ICC_TEST_SEND_CNT; i++)
	{
		if (ICC_SEND_SYNC_RET != bsp_icc_send_sync((u32)ICC_SEND_CPU, channel_id,(u8*)&send_data,(u32)sizeof(send_data), MAX_SCHEDULE_TIMEOUT)) /*lint !e40 !e516*/
		{
...
static s32 icc_test_multi_send_wait_ack_init(u32 flag)
{
	...
	if(ICC_ERR == osl_task_init("icc_wait", ICC_TEST_TASK_PRI+1, ICC_TASK_STK_SIZE,
			(void*)icc_test_wait, (void*)(u64)(flag), (void*)&g_icc_test.task_id))
    {
...
if(ICC_ERR == osl_task_init((char *)name, ICC_TEST_TASK_PRI, ICC_TASK_STK_SIZE,
		(void*)entry, (void*)(unsigned long)(channel_id), (void*)&task_id))
    {
...
s32 bsp_icc_test_init(void)
{
	...
	if(ICC_ERR == osl_task_init("icc_read_test", ICC_TEST_TASK_PRI+1, ICC_TASK_STK_SIZE,
			(task_entry)icc_read_task_entry, (void *)(ICC_CHN_CSHELL << 16 | 2), (void *)&g_icc_test.recv_task_id))
    {
...
s32 icc_send_test_002(void)
{
	...
	if (ICC_SEND_SYNC_RET == (u32)bsp_icc_send_sync(ICC_SEND_CPU, channel_id, (u8*)&send_data, (u32)sizeof(send_data), MAX_SCHEDULE_TIMEOUT))/*lint !e40 !e516*/
	{
...
s32 icc_send_test_009(void)
{
	...
	if (ICC_SEND_SYNC_RET == (u32)bsp_icc_send_sync(ICC_SEND_CPU, channel_id, (u8*)&send_data, (u32)0, MAX_SCHEDULE_TIMEOUT))/*lint !e40 !e516*/
	{
		icc_print_info("[PASS] single channel, synchronous send!\n");
		return ICC_TEST_PASS;
	}
...
void icc_packet_dump_test(void)
{
	struct icc_channel_packet packet = {0};

	packet.channel_id = 0x1;
	icc_channel_packet_dump(&packet);

	packet.channel_id = 0x20;
	icc_channel_packet_dump(&packet);

	packet.channel_id = 0x10020020;
	icc_channel_packet_dump(&packet);

	packet.channel_id = 0x000A0001;
	icc_channel_packet_dump(&packet);

	packet.channel_id = 0x000B0001;
	icc_channel_packet_dump(&packet);
}
