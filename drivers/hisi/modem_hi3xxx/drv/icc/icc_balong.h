...
static inline void wake_lock_init(struct wake_lock *lock, int lock_id, const char *name){}
...
struct icc_channel
{
	...
	struct task_struct *               private_task_id;  /* 通道私有的处理任务id */
...
struct icc_control
{
	...
	struct task_struct *        shared_task_id;            /* 通道共享任务id */
