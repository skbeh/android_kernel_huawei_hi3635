...
u32 fifo_put_with_header(struct icc_channel_fifo *fifo, u8 *head_buf, u32 head_len, u8 *data_buf, u32 data_len)
{
	...
	if (tail_idle_size < (s32)head_len)
	{
		...
		memcpy(base_addr, (void *)(head_buf + tail_idle_size), (head_len - (u32)tail_idle_size));
		write          = head_len - (u32)tail_idle_size;
		tail_idle_size = 0;
	}
	else
	{
		memcpy(base_addr + write, (void *)head_buf, head_len);
	...
	{
		memcpy(base_addr + write, (void *)data_buf, (u32)tail_idle_size);
		memcpy(base_addr, (void *)(data_buf + tail_idle_size), data_len - (u32)tail_idle_size);
...
u32 fifo_get(struct icc_channel_fifo *fifo, u8 *data_buf, u32 data_len, u32 *read)
{
	...
	if(readed_len > data_len)
	{
		...
		(*(unsigned *) (ICC_DBG_MSG_ADDR_IN_DDR + sizeof(base_addr) )) = (write);
		(*(unsigned *) (ICC_DBG_MSG_ADDR_IN_DDR + sizeof(base_addr)+4)) = (*read);
		(*(unsigned *) (ICC_DBG_MSG_ADDR_IN_DDR + sizeof(base_addr)+8)) = (buf_len);
	...
	{
		memcpy((void *)data_buf, base_addr + *read, readed_len);
		*read += readed_len;
	}
	else
	{
		memcpy((void *)data_buf, base_addr + *read, (u32)tail_idle_size);
...
#endif
{
	...
	for( ; ;)
	{
		...
		wake_unlock(&g_icc_ctrl.wake_lock);
...
s32 bsp_icc_send(u32 cpuid, u32 channel_id, u8* data, u32 data_len)
{
	...
	{
		icc_print_error("invalid parameter! cpuid=0x%x, channel_id=0x%x, data=%p, data_len=0x%x\n", cpuid, channel_id, data, data_len);
		return ICC_INVALID_PARA;
	}
...
out:
	icc_print_error("errorno=0x%x, cpuid=0x%x, channel_id=0x%x, data=%p, data_len=0x%x\n",
		ret, cpuid, channel_id, data, data_len);
...
out:
	icc_print_error("errno=0x%x, buffer=0x%p, len%d ?= %dread_len\n", ret, buf, buf_len, read_len);
...
#ifdef __KERNEL__
	kthread_stop(g_icc_ctrl.shared_task_id); /*lint !e64 */
