...
BSP_BOOL bsp_ptr_invalid(const void* pMem)
{
    ...
    for (u32Type = MEM_NORM_DDR_POOL; u32Type < MEM_POOL_MAX; u32Type++)
    {
        ...
        if ((u32)pRel >= pPoolInfo->u32BaseAddr ||
            (u32)pRel < pPoolInfo->u32BaseAddr + pPoolInfo->u32Size)
        {
            u32FindMem = 1;
        }
    }
...
void* bsp_pool_alloc(MEM_ALLOC_INFO* pAllocInfo, u32 u32Size)
{
   ...
   {
   	    pvRetAddr = (void*)SHD_DDR_P2V(u32RetAddr);
   }
   else
   {
   	return NULL;
...
s32 bsp_mem_dump_alloc_list(u32 u32Size, u32 u32PoolType)
{
    ...
    u32CurAddr = (u32)SHD_DDR_V2P(pAllocInfo->allocList[0]);


    for (;NULL != pCurAddr; u32CurAddr = MEM_ITEM_NEXT(pCurAddr), pCurAddr = (void *)(unsigned long)SHD_DDR_P2V(u32CurAddr))
    {
...
s32 bsp_mem_scan(u32 u32PoolType)
{
    ...
    u32CurScan =  (void*)SHD_DDR_P2V(pAllocInfo->memPoolInfo.u32BaseAddr);
    u32MgrSize = pAllocInfo->memPoolInfo.u32MgrSize;
    u32EndAddr = (void*)SHD_DDR_P2V(pAllocInfo->memPoolInfo.u32CurPosAddr);
